...
namespace nogdb {
  ...
  Class::createExtend(const Txn &txn, const std::string &className, const std::string &superClass) {
    BEGIN_VALIDATION(&txn)
        .isTransactionValid()
        .isClassIdMaxReach()
        .isClassNameValid(className)
  ...
  void Class::drop(const Txn &txn, const std::string &className) {
    ...
    try {
      ...
      txn._dbInfo->setNumClassId(txn._dbInfo->getNumClassId() - ClassId{1});
      txn._dbInfo->setNumPropertyId(
          txn._dbInfo->getNumPropertyId() - PropertyId{static_cast<uint16_t>(propertyInfos.size())});
    } catch (const Error &err) {
      txn.rollback();
      throw NOGDB_FATAL_ERROR(err);
    } catch (...) {
