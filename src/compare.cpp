...
//*****************************************************************
    ...
    ResultSet Compare::compareCondition(const Txn &txn,
        ...
        if (foundIndex.first) {
            ...
            auto result = ResultSet{};
            for(const auto& recordDescriptor: indexedRecords) {
                result.emplace_back(Result{recordDescriptor, txn._iRecord->getRecord(classInfo, recordDescriptor)});
            }
        }
        if (searchIndexOnly) {
            return ResultSet{};
        } else {
            return getRecordCondition(txn, classInfo, condition);
        }
    }
