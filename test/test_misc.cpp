...
void test_delete_vertex_with_edges() {
	...
	try {
		...
		assert(record.empty());
		record = nogdb::Db::getRecord(txn, v1_1);
		assert(!record.empty());
		record = nogdb::Db::getRecord(txn, v1_2);
		assert(!record.empty());
		record = nogdb::Db::getRecord(txn, e1);
		assert(record.empty());
		record = nogdb::Db::getRecord(txn, e2);
		assert(record.empty());
...
void test_drop_and_find_extended_class() {
	...
	try {
		...
		nogdb::Vertex::create(txn, "vertex3", nogdb::Record{}.set("prop0", 0U).set("prop1", 1U).set("prop2", 1).set("prop3", 1.1));
		nogdb::Vertex::create(txn, "vertex4", nogdb::Record{}.set("prop0", 0U).set("prop1", 1U).set("prop2", 1).set("prop3", "hello"));
		txn.commit();
	} catch(const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
		assert(false);
