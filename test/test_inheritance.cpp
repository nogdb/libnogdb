...
void assert_class(const nogdb::Txn& txn, const std::string& className, const std::string& superClassName, size_t sizeOfSubClasses, size_t sizeOfProperties) {
...
void test_create_vertex_edge_extend() {
	auto txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_WRITE};
	try {
		auto v1 = nogdb::Vertex::create(txn, "infras", nogdb::Record{}.set("name", "Peter").set("js_skills", 7).set("IT_skills", 9));
...
void test_get_class_extend() {
	...
	try {
		...
		b = nogdb::Vertex::create(txn, "backends", nogdb::Record{}.set("name", "Bill").set("age", 32U).set("cpp_skills", 7));
		c = nogdb::Vertex::create(txn, "systems", nogdb::Record{}.set("name", "Charon").set("age", 27U).set("js_skills", 6).set("cpp_skills", 8).set("devops_skills", 10));
		...
		f = nogdb::Vertex::create(txn, "frontends", nogdb::Record{}.set("name", "Falcao").set("age", 34U).set("js_skills", 9));
...
void test_find_class_extend() {
	...
	try {
		...
		res = nogdb::Vertex::getInEdge(txn, b[0].descriptor, nogdb::Condition("name").endWith("provider").ignoreCase(), nogdb::ClassFilter{"collaborate"});
		assert(res.size() == 2);
		assert(res[0].record.get("name").toText() == "ui provider" || res[0].record.get("name").toText() == "system provider");
		assert(res[1].record.get("name").toText() == "ui provider" || res[1].record.get("name").toText() == "system provider");
		res = nogdb::Vertex::getInEdge(txn, b[0].descriptor, nogdb::Condition("type").null(), nogdb::ClassFilter{"inter", "manage"});
		assert(res.size() == 2);
		...
		assert(c.size() == 1);
		res = nogdb::Vertex::getOutEdge(txn, c[0].descriptor, nogdb::Condition("name").beginWith("team").ignoreCase(), nogdb::ClassFilter{"action"});
		assert(res.size() == 1);
		assert(res[0].record.get("name").toText() == "team leader");
		res = nogdb::Vertex::getAllEdge(txn, b[0].descriptor, nogdb::Condition("name").contain("team").ignoreCase(), nogdb::ClassFilter{"collaborate"});
		assert(res.size() == 1);
...
void test_shortest_path_class_extend() {
	...
	try {
		...
		res = nogdb::Traverse::shortestPath(txn, b[0].descriptor, d[0].descriptor, nogdb::ClassFilter{"inter", "manage"});
		assert(res.size() == 0);
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
		assert(false);
