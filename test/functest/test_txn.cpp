...
void test_txn_create_only_vertex_rollback() {
  ...
  try {
    ...
    nogdb::Transaction txnRo{*ctx, nogdb::Txn::Mode::READ_ONLY};
    nogdb::Transaction txnRw00{*ctx, nogdb::Txn::Mode::READ_WRITE};
...
void test_txn_get_vertex_edge() {
  ...
  try {
    ...
    std::vector<std::function<void(nogdb::Transaction &)>> testCases{
        ...
        [&](const nogdb::Transaction &txn) {
          auto res = nogdb::Vertex::getOutEdge(txn, v1, nogdb::GraphFilter{}.only("bridge"));
          assert(res[0].record.get("name").toText() == "12");
        },
        [&](const nogdb::Transaction &txn) {
          auto res = nogdb::Vertex::getOutEdge(txn, v1, nogdb::GraphFilter{}.only("flight"));
          assert(res[0].record.get("name").toText() == "13");
        }
    };
...
void test_txn_alter_vertex_edge_commit() {
  ...
  try {
    ...
    std::vector<std::function<void(nogdb::Transaction &)>> newTestCases{
        ...
        [&](const nogdb::Transaction &txn) {
          auto res = nogdb::Vertex::getInEdge(txn, v2, nogdb::GraphFilter{}.only("bridge"));
          assert(res[0].descriptor == e1);
        },
        [&](const nogdb::Transaction &txn) {
          auto res = nogdb::Vertex::getInEdge(txn, v2, nogdb::GraphFilter{}.only("flight"));
          assert(res[0].descriptor == e2);
        }
    };
