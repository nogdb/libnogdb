...
void test_dfs_traverse_in() {
  ...
  try {
    for (const auto &res: nogdb::Traverse::inEdgeDfs(txn, D, 0, 1, nogdb::GraphFilter{}.only("link"))) {
      auto name = res.record.get("name").toText();
    ...
    for (const auto &res: nogdb::Traverse::inEdgeDfs(txn, D, 1, 2, nogdb::GraphFilter{}.only("link"))) {
      auto name = res.record.get("name").toText();
      assert(name == "B" || name == "A");
      if (name == "B") assert(res.record.getDepth() == 1);
      else
...
void test_dfs_traverse_all() {
  ...
  try {
    auto count = 0;
    for (const auto &res: nogdb::Traverse::allEdgeDfs(txn, F, 0, 1, nogdb::GraphFilter{}.only("link"))) {
      auto name = res.record.get("name").toText();
    ...
    count = 0;
    for (const auto &res: nogdb::Traverse::allEdgeDfs(txn, F, 1, 2, nogdb::GraphFilter{}.only("link"))) {
      auto name = res.record.get("name").toText();
    ...
    count = 0;
    for (const auto &res: nogdb::Traverse::allEdgeDfs(txn, F, 1, 3, nogdb::GraphFilter{}.only("link"))) {
      auto name = res.record.get("name").toText();
      assert(name == "d" || name == "C" || name == "H" || name == "e" ||
             name == "A" || name == "c" || name == "a" || name == "B");
      count++;
  ...
  try {
    auto count = 0;
    for (const auto &res: nogdb::Traverse::allEdgeDfs(txn, H, 1, 1, nogdb::GraphFilter{}.only("symbolic"))) {
      auto name = res.record.get("name").toText();
      assert(name == "C");
      count++;
    }
    ...
    count = 0;
    for (const auto &res: nogdb::Traverse::allEdgeDfs(txn, H, 1, 2, nogdb::GraphFilter{}.only("symbolic"))) {
      auto name = res.record.get("name").toText();
  ...
  try {
    auto count = 0;
    for (const auto &res: nogdb::Traverse::allEdgeDfs(txn, A, 1, 1, nogdb::GraphFilter{}.only("link", "symbolic"))) {
      auto name = res.record.get("name").toText();
      assert(name == "B" || name == "a" || name == "C" || name == "D");
      count++;
    }
...
void test_invalid_dfs_traverse_all() {
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::allEdgeDfs(txn, A, 0, 0, nogdb::GraphFilter{}.only("ling"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_NOEXST_CLASS, "NOGDB_CTX_NOEXST_CLASS");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::allEdgeDfs(txn, A, 0, 0, nogdb::GraphFilter{}.only("link", "symbol"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_NOEXST_CLASS, "NOGDB_CTX_NOEXST_CLASS");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::allEdgeDfs(txn, A, 0, 0, nogdb::GraphFilter{}.only("folders"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_MISMATCH_CLASSTYPE, "NOGDB_CTX_MISMATCH_CLASSTYPE");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::allEdgeDfs(txn, A, 0, 0, nogdb::GraphFilter{}.only("link", "folders"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_MISMATCH_CLASSTYPE, "NOGDB_CTX_MISMATCH_CLASSTYPE");
...
void test_dfs_traverse_in_cursor() {
  ...
  try {
    auto rsCursor = nogdb::Traverse::inEdgeDfsCursor(txn, D, 0, 1, nogdb::GraphFilter{}.only("link"));
    assert(rsCursor.size() == 2);
    while (rsCursor.next()) {
      auto name = rsCursor->record.get("name").toText();
      assert(name == "B" || name == "D");
    ...
    rsCursor = nogdb::Traverse::inEdgeDfsCursor(txn, D, 1, 2, nogdb::GraphFilter{}.only("link"));
    assert(rsCursor.size() == 2);
    while (rsCursor.next()) {
      auto name = rsCursor->record.get("name").toText();
      assert(name == "B" || name == "A");
    ...
    rsCursor = nogdb::Traverse::inEdgeDfsCursor(txn, D, 3, 4, nogdb::GraphFilter{}.only("link"));
    assert(rsCursor.empty());

    rsCursor = nogdb::Traverse::inEdgeDfsCursor(txn, D, 0, 0, nogdb::GraphFilter{}.only("link"));
    while (rsCursor.next()) {
      auto name = rsCursor->record.get("name").toText();
      assert(name == "D");
    }
...
void test_dfs_traverse_all_cursor() {
  ...
  try {
    auto res = nogdb::Traverse::allEdgeDfsCursor(txn, F, 0, 1, nogdb::GraphFilter{}.only("link"));
    while (res.next()) {
      auto name = res->record.get("name").toText();
      assert(name == "d" || name == "C" || name == "H" || name == "e" || name == "F");
    }
    ...
    res = nogdb::Traverse::allEdgeDfsCursor(txn, F, 1, 2, nogdb::GraphFilter{}.only("link"));
    while (res.next()) {
      auto name = res->record.get("name").toText();
      assert(name == "d" || name == "C" || name == "H" || name == "e" || name == "A" || name == "c");
    }
    ...
    res = nogdb::Traverse::allEdgeDfsCursor(txn, F, 1, 3, nogdb::GraphFilter{}.only("link"));
    while (res.next()) {
      auto name = res->record.get("name").toText();
      assert(name == "d" || name == "C" || name == "H" || name == "e" ||
             name == "A" || name == "c" || name == "a" || name == "B");
  ...
  try {
    auto res = nogdb::Traverse::allEdgeDfsCursor(txn, H, 1, 1, nogdb::GraphFilter{}.only("symbolic"));
    while (res.next()) {
      auto name = res->record.get("name").toText();
      assert(name == "C");
    }
    ...
    res = nogdb::Traverse::allEdgeDfsCursor(txn, H, 1, 2, nogdb::GraphFilter{}.only("symbolic"));
    while (res.next()) {
      auto name = res->record.get("name").toText();
      assert(name == "C" || name == "e");
    }
  ...
  try {
    auto res = nogdb::Traverse::allEdgeDfsCursor(txn, A, 1, 1, nogdb::GraphFilter{}.only("link", "symbolic"));
    while (res.next()) {
      auto name = res->record.get("name").toText();
      assert(name == "B" || name == "a" || name == "C" || name == "D");
    }
...
void test_invalid_dfs_traverse_in_cursor() {
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::inEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("ling"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_NOEXST_CLASS, "NOGDB_CTX_NOEXST_CLASS");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::inEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("link", "symbol"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_NOEXST_CLASS, "NOGDB_CTX_NOEXST_CLASS");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::inEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("folders"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_MISMATCH_CLASSTYPE, "NOGDB_CTX_MISMATCH_CLASSTYPE");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::inEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("link", "folders"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_MISMATCH_CLASSTYPE, "NOGDB_CTX_MISMATCH_CLASSTYPE");
...
void test_invalid_dfs_traverse_out_cursor() {
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::outEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("ling"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_NOEXST_CLASS, "NOGDB_CTX_NOEXST_CLASS");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::outEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("link", "symbol"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_NOEXST_CLASS, "NOGDB_CTX_NOEXST_CLASS");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::outEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("folders"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_MISMATCH_CLASSTYPE, "NOGDB_CTX_MISMATCH_CLASSTYPE");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::outEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("link", "folders"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_MISMATCH_CLASSTYPE, "NOGDB_CTX_MISMATCH_CLASSTYPE");
...
void test_invalid_dfs_traverse_all_cursor() {
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::allEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("ling"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_NOEXST_CLASS, "NOGDB_CTX_NOEXST_CLASS");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::allEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("link", "symbol"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_NOEXST_CLASS, "NOGDB_CTX_NOEXST_CLASS");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::allEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("folders"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_MISMATCH_CLASSTYPE, "NOGDB_CTX_MISMATCH_CLASSTYPE");
  ...
  txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
  try {
    auto res = nogdb::Traverse::allEdgeDfsCursor(txn, A, 0, 0, nogdb::GraphFilter{}.only("link", "folders"));
    assert(false);
  } catch (const nogdb::Error &ex) {
    txn.rollback();
    REQUIRE(ex, NOGDB_CTX_MISMATCH_CLASSTYPE, "NOGDB_CTX_MISMATCH_CLASSTYPE");
