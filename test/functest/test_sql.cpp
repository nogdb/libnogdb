...
void test_sql_create_class_extend() {
  ...
  try {
    ...
    for (const auto &property: properties) {
      ...
      else if (property.name == "prop2") assert(property.type == nogdb::PropertyType::UNSIGNED_INTEGER);
      else
        assert(false);
    }
  } catch (const Error &ex) {
...
void test_sql_select_method_property() {
  ...
  try {
    ...
    result = SQL::execute(txn, "SELECT propV, out('e')[2].propV, outE()[propE='e1->5'].inV().propV as out_propV FROM " +
                               to_string(v1));
    assert(result.type() == result.RESULT_SET);
    res = result.get<ResultSet>();
    assert(res[0].descriptor == RecordDescriptor(-2, 0));
