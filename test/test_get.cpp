...
void test_find_vertex() {
	...
	try {
		...
		assert(res.size() == 0);
		res = nogdb::Vertex::get(txn, "locations", nogdb::Condition("population").eq(static_cast<unsigned long long>(2000)));
		assert(res.size() == 1);
		assert(res[0].record.get("name").toText() == "New York Tower");
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
...
void test_find_edge_in() {
	...
	try {
		...
		auto res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("George Street"), nogdb::ClassFilter{"street"});
		assert(res.size() == 1);
		res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("distance").gt(40.0), nogdb::ClassFilter{"street"});
		assert(res.size() == 2);
	...
	try {
		...
		auto res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("Isaac Street"), classNames);
		assert(res.size() == 1);
		res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("distance").lt(200.0), classNames);
		assert(res.size() == 3);
		std::sort(res.begin(), res.end(), cmp);
	...
	try {
		...
		assert(vertices.size() == 1);
		auto &vertex = vertices[0];
		auto res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("The Outer Ring C"));
		assert(res.size() == 1);
		res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("distance").ge(36.2));
		assert(res.size() == 3);
		std::sort(res.begin(), res.end(), cmp);
...
void test_find_edge_out() {
	...
	try {
		...
		assert(vertices.size() == 1);
		auto &vertex = vertices[0];
		auto res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(res.size() == 1);
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("distance").ge(100.0), nogdb::ClassFilter{"railway"});
		assert(res.size() == 3);
		std::sort(res.begin(), res.end(), cmp);
		...
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("temperature").le(42), nogdb::ClassFilter{"railway"});
		assert(res.size() == 2);
		std::sort(res.begin(), res.end(), cmp);
		assert(res[0].record.get("name").toText() == "Andy Way");
		assert(res[1].record.get("name").toText() == "Catalina Way");
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, !nogdb::Condition("temperature"), nogdb::ClassFilter{"railway"});
		assert(res.size() == 1);
		assert(res[0].record.get("name").toText() == "Dwayne Way");
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
	...
	try {
		...
		auto &vertex = vertices[0];
		auto classNames = nogdb::ClassFilter{"street", "railway"};
		auto res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, !nogdb::Condition("name").eq("Andrew Street"), classNames);
		assert(res.size() == 5);
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, !nogdb::Condition("name").contain("boo"), classNames);
		assert(res.size() == 5);
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("name").contain("BOO").ignoreCase(), classNames);
		assert(res.size() == 1);
		assert(res[0].record.get("name").toText() == "Bamboo Way");
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, !nogdb::Condition("name").beginWith("a").ignoreCase(), classNames);
		assert(res.size() == 4);
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("name").beginWith("A"), classNames);
		assert(res.size() == 2);
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
		assert(false);
	...
	try {
		...
		assert(vertices.size() == 1);
		auto &vertex = vertices[0];
		auto res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("The Outer Ring B"));
		assert(res.size() == 1);
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, !nogdb::Condition("name").endWith("StrEEt").ignoreCase());
		assert(res.size() == 6);
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("name").endWith("Way"));
		assert(res.size() == 4);
		std::sort(res.begin(), res.end(), cmp);
		assert(res[0].record.get("name").toText() == "Andy Way");
		...
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, !nogdb::Condition("coordinates").null());
		assert(res.size() == 4);
		std::sort(res.begin(), res.end(), cmp);
		assert(res[0].record.get("name").toText() == "Andrew Street");
...
void test_find_edge_all() {
	...
	try {
		...
		assert(vertices.size() == 1);
		auto &vertex = vertices[0];
		auto res = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("George Street"), nogdb::ClassFilter{"street"});
		assert(res.size() == 1);
		res = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, nogdb::Condition("distance").ge(50.0), nogdb::ClassFilter{"street"});
		assert(res.size() == 2);
		std::sort(res.begin(), res.end(), cmp);
	...
	try {
		...
		auto &vertex = vertices[0];
		auto classNames = nogdb::ClassFilter{"street", "railway"};
		auto res = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, nogdb::Condition("distance").gt(100.0), classNames);
		assert(res.size() == 1);
		assert(res[0].record.get("name").toText() == "Gravity Way");
		res = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, nogdb::Condition("distance").le(100.0), classNames);
		assert(res.size() == 5);
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
		assert(false);
	...
	try {
		...
		assert(vertices.size() == 1);
		auto &vertex = vertices[0];
		auto res = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, nogdb::Condition("capacity").eq(100U));
		assert(res.size() == 1);
		assert(res[0].record.get("name").toText() == "Doodee Street");
		res = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, nogdb::Condition("name").contain("C"));
		assert(res.size() == 2);
		std::sort(res.begin(), res.end(), cmp);
...
void test_find_invalid_edge_in() {
	...
	auto &vertex = vertices[0];
	try {
		nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		...
		nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("names").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_PROPERTY, "CTX_NOEXST_PROPERTY");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("coordinates").contain("a"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_INVALID_COMPARATOR, "CTX_INVALID_COMPARATOR");
	...
	auto &edge = edges[0];
	try {
		nogdb::Vertex::getInEdge(txn, edge.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		...
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getInEdge(txn, tmp, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
...
void test_find_invalid_edge_out() {
	...
	auto &vertex = vertices[0];
	try {
		auto edges = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		auto edges = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("names").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_PROPERTY, "CTX_NOEXST_PROPERTY");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("coordinates").contain("a"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_INVALID_COMPARATOR, "CTX_INVALID_COMPARATOR");
	...
	auto &edge = edges[0];
	try {
		auto edges = nogdb::Vertex::getOutEdge(txn, edge.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getOutEdge(txn, tmp, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_invalid_edge_all() {
	...
	auto &vertex = vertices[0];
	try {
		auto edges = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		auto edges = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, nogdb::Condition("names").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_PROPERTY, "CTX_NOEXST_PROPERTY");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, nogdb::Condition("coordinates").contain("a"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_INVALID_COMPARATOR, "CTX_INVALID_COMPARATOR");
	...
	auto &edge = edges[0];
	try {
		auto edges = nogdb::Vertex::getAllEdge(txn, edge.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getAllEdge(txn, tmp, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_edge_all_with_expression() {
	...
	try {
		...
		assert(vertices.size() == 1);
		auto &vertex = vertices[0];
		auto cond1 = nogdb::Condition("@className").eq("street") and nogdb::Condition("name").contain("street").ignoreCase();
...
void test_find_edge_in_condition_function() {
	...
	try {
		...
		auto res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("George Street"), nogdb::ClassFilter{"street"});
		assert(res.size() == 1);
		res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, test_condition_function_4, nogdb::ClassFilter{"street"});
		assert(res.size() == 2);
		std::sort(res.begin(), res.end(), cmp);
		assert(res[0].record.get("name").toText() == "Andrew Street");
		assert(res[1].record.get("name").toText() == "George Street");
	...
	try {
		...
		auto res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("Isaac Street"), classNames);
		assert(res.size() == 1);
		res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, test_condition_function_5, classNames);
		assert(res.size() == 3);
		std::sort(res.begin(), res.end(), cmp);
		assert(res[0].record.get("name").toText() == "Dwayne Way");
		assert(res[1].record.get("name").toText() == "Henry Road");
	...
	try {
		...
		auto res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("The Outer Ring C"));
		assert(res.size() == 1);
		res = nogdb::Vertex::getInEdge(txn, vertex.descriptor, test_condition_function_6);
		assert(res.size() == 3);
		std::sort(res.begin(), res.end(), cmp);
		assert(res[0].record.get("name").toText() == "Eastern Way");
		assert(res[1].record.get("name").toText() == "Fisher Avenue");
...
void test_find_edge_out_condition_function() {
	...
	try {
		...
		auto res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(res.size() == 1);
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, test_condition_function_7, nogdb::ClassFilter{"railway"});
		assert(res.size() == 3);
		std::sort(res.begin(), res.end(), cmp);
		assert(res[0].record.get("name").toText() == "Bamboo Way");
		assert(res[1].record.get("name").toText() == "Catalina Way");
		assert(res[2].record.get("name").toText() == "Dwayne Way");
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, test_condition_function_8, nogdb::ClassFilter{"railway"});
		assert(res.size() == 2);
		std::sort(res.begin(), res.end(), cmp);
		assert(res[0].record.get("name").toText() == "Andy Way");
		assert(res[1].record.get("name").toText() == "Catalina Way");
		res = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, test_condition_function_9, nogdb::ClassFilter{"railway"});
		assert(res.size() == 1);
		assert(res[0].record.get("name").toText() == "Dwayne Way");
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
...
void test_find_edge_all_condition_function() {
	...
	try {
		...
		auto &vertex = vertices[0];
		auto classNames = nogdb::ClassFilter{"street", "railway"};
		auto res = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, test_condition_function_10, classNames);
		assert(res.size() == 1);
		assert(res[0].record.get("name").toText() == "Gravity Way");
		res = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, test_condition_function_11, classNames);
		assert(res.size() == 5);
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
		assert(false);
...
void test_find_invalid_edge_in_condition_function() {
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getInEdge(txn, vertex.descriptor, condition, nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		auto edges = nogdb::Vertex::getInEdge(txn, vertex.descriptor, condition, classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	auto &edge = edges[0];
	try {
		auto edges = nogdb::Vertex::getInEdge(txn, edge.descriptor, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getInEdge(txn, tmp, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_invalid_edge_out_condition_function() {
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, condition, nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		auto edges = nogdb::Vertex::getOutEdge(txn, vertex.descriptor, condition, classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	auto &edge = edges[0];
	try {
		auto edges = nogdb::Vertex::getOutEdge(txn, edge.descriptor, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getOutEdge(txn, tmp, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_invalid_edge_all_condition_function() {
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, condition, nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		auto edges = nogdb::Vertex::getAllEdge(txn, vertex.descriptor, condition, classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	auto &edge = edges[0];
	try {
		auto edges = nogdb::Vertex::getAllEdge(txn, edge.descriptor, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getAllEdge(txn, tmp, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_vertex_cursor() {
	...
	try {
		...
		assert(res.size() == 1);
		auto tmp = Coordinates{};
		res.first(); res->record.get("coordinates").convertTo(tmp);
		assert(tmp.x == -1.00);
		assert(tmp.y == 1.00);
		...
		assert(res.size() == 1);
		res.first(); assert(res->record.get("name").toText() == "Pentagon");
		res = nogdb::Vertex::getCursor(txn, "locations", nogdb::Condition("postcode").eq(11600U));
		assert(res.size() == 1);
		res.first(); assert(res->record.get("name").toText() == "ThaiCC Tower");
		res = nogdb::Vertex::getCursor(txn, "locations", nogdb::Condition("price").eq(280000LL));
		assert(res.size() == 1);
		res.first(); assert(res->record.get("name").toText() == "Dubai Building");
		res = nogdb::Vertex::getCursor(txn, "locations", nogdb::Condition("population").eq(900ULL));
		assert(res.size() == 2);
		res.next(); assert(res->record.get("name").toText() == "ThaiCC Tower");
		res.next(); assert(res->record.get("name").toText() == "Pentagon");
		res = nogdb::Vertex::getCursor(txn, "locations", nogdb::Condition("rating").eq(4.5));
		assert(res.size() == 2);
		res.next(); assert(res->record.get("name").toText() == "New York Tower");
		res.next(); assert(res->record.get("name").toText() == "Empire State Building");
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
		assert(false);
	}
	...
	try {
		...
		assert(res.size() == 1);
		res.last(); assert(res->record.get("name").toText() == "ThaiCC Tower");
		...
		assert(res.size() == 0);
		res = nogdb::Vertex::getCursor(txn, "locations", nogdb::Condition("population").eq(static_cast<unsigned long long>(2000)));
		assert(res.size() == 1);
		res.last(); assert(res->record.get("name").toText() == "New York Tower");
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
		assert(false);
	}
	...
	try {
		...
		assert(res.size() == 1);
		res.to(0); assert(res->record.get("name").toText() == "Pentagon");
		...
		assert(res.size() == 1);
		res.to(0); assert(res->record.get("name").toText() == "ThaiCC Tower");
...
void test_find_vertex_cursor_condition_function() {
	...
	try {
		...
		assert(res.size() == 3);
		res.next(); assert(res->record.get("name").toText() == "New York Tower");
		res.next(); assert(res->record.get("name").toText() == "Dubai Building");
		res.next(); assert(res->record.get("name").toText() == "Empire State Building");
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
		assert(false);
	...
	try {
		...
		assert(res.size() == 4);
		res.first(); assert(res->record.get("name").toText() == "New York Tower");
		res.to(1); assert(res->record.get("name").toText() == "Dubai Building");
		res.to(2); assert(res->record.get("name").toText() == "ThaiCC Tower");
		res.last(); assert(res->record.get("name").toText() == "Pentagon");
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
		assert(false);
	}
...
void test_find_edge_cursor_condition_function() {
	...
	try {
		...
		assert(res.size() == 1);
		auto tmp = Coordinates{};
		res.next(); res->record.get("coordinates").convertTo(tmp);
		assert(tmp.x == 0.1);
		assert(tmp.y == -0.1);
	} catch (const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
...
void test_find_edge_in_cursor() {
    ...
    try {
        ...
        auto res = nogdb::Vertex::getInEdgeCursor(txn, vertex, nogdb::Condition("name").eq("George Street"), nogdb::ClassFilter{"street"});
        assert(res.size() == 1);
        res = nogdb::Vertex::getInEdgeCursor(txn, vertex, nogdb::Condition("distance").gt(40.0), nogdb::ClassFilter{"street"});
        assert(res.count() == 2);
    ...
    try {
        ...
        auto &vertex = vertices->descriptor;
        auto classNames = nogdb::ClassFilter{"street", "railway"};
        auto res = nogdb::Vertex::getInEdgeCursor(txn, vertex, nogdb::Condition("name").eq("Isaac Street"), classNames);
        assert(res.size() == 1);
        res = nogdb::Vertex::getInEdgeCursor(txn, vertex, nogdb::Condition("distance").lt(200.0), classNames);
        assert(res.count() == 3);
...
void test_find_edge_out_cursor() {
    ...
    try {
        ...
        vertices.next();
        auto &vertex = vertices->descriptor;
        auto res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
        assert(res.size() == 1);
        res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, nogdb::Condition("distance").ge(100.0), nogdb::ClassFilter{"railway"});
        assert(res.size() == 3);
        cursorContains(res, std::set<std::string>{"Bamboo Way", "Catalina Way", "Dwayne Way"}, "name");
        res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, nogdb::Condition("temperature").le(42), nogdb::ClassFilter{"railway"});
        assert(res.size() == 2);
        cursorContains(res, std::set<std::string>{"Andy Way", "Catalina Way"}, "name");
        res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, !nogdb::Condition("temperature"), nogdb::ClassFilter{"railway"});
        assert(res.size() == 1);
    ...
    try {
        ...
        auto &vertex = vertices->descriptor;
        auto classNames = nogdb::ClassFilter{"street", "railway"};
        auto res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, !nogdb::Condition("name").eq("Andrew Street"), classNames);
        assert(res.size() == 5);
        res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, !nogdb::Condition("name").contain("boo"), classNames);
        assert(res.size() == 5);
        res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, nogdb::Condition("name").contain("BOO").ignoreCase(), classNames);
        assert(res.size() == 1);
        cursorContains(res, std::set<std::string>{"Bamboo Way"}, "name");
        res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, !nogdb::Condition("name").beginWith("a").ignoreCase(), classNames);
        assert(res.size() == 4);
        res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, nogdb::Condition("name").beginWith("A"), classNames);
        assert(res.size() == 2);
    } catch (const nogdb::Error& ex) {
        std::cout << "\nError: " << ex.what() << std::endl;
        assert(false);
...
void test_find_edge_all_cursor() {
    ...
    try {
        ...
        vertices.next();
        auto &vertex = vertices->descriptor;
        auto res = nogdb::Vertex::getAllEdgeCursor(txn, vertex, nogdb::Condition("name").eq("George Street"), nogdb::ClassFilter{"street"});
        assert(res.size() == 1);
        res = nogdb::Vertex::getAllEdgeCursor(txn, vertex, nogdb::Condition("distance").ge(50.0), nogdb::ClassFilter{"street"});
        assert(res.size() == 2);
    ...
    try {
        ...
        auto &vertex = vertices->descriptor;
        auto classNames = nogdb::ClassFilter{"street", "railway"};
        auto res = nogdb::Vertex::getAllEdgeCursor(txn, vertex, nogdb::Condition("distance").gt(100.0), classNames);
        assert(res.size() == 1);
        res.next(); assert(res->record.get("name").toText() == "Gravity Way");
        res = nogdb::Vertex::getAllEdgeCursor(txn, vertex, nogdb::Condition("distance").le(100.0), classNames);
        assert(res.size() == 5);
    } catch (const nogdb::Error& ex) {
        std::cout << "\nError: " << ex.what() << std::endl;
        assert(false);
    ...
    try {
        ...
        assert(res.size() == 1);
        res.first(); assert(res->record.get("name").toText() == "Doodee Street");
...
void test_find_invalid_edge_in_cursor() {
	...
	auto &vertex = vertices[0];
	try {
		nogdb::Vertex::getInEdgeCursor(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		nogdb::Vertex::getInEdgeCursor(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getInEdgeCursor(txn, vertex.descriptor, nogdb::Condition("names").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_PROPERTY, "CTX_NOEXST_PROPERTY");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getInEdgeCursor(txn, vertex.descriptor, nogdb::Condition("distance").contain("a"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_INVALID_COMPARATOR, "CTX_INVALID_COMPARATOR");
	...
	auto &edge = edges[0];
	try {
		nogdb::Vertex::getInEdgeCursor(txn, edge.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getInEdgeCursor(txn, tmp, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_invalid_edge_out_cursor() {
	...
	auto &vertex = vertices[0];
	try {
		auto edges = nogdb::Vertex::getOutEdgeCursor(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		auto edges = nogdb::Vertex::getOutEdgeCursor(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getOutEdgeCursor(txn, vertex.descriptor, nogdb::Condition("names").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_PROPERTY, "CTX_NOEXST_PROPERTY");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getOutEdgeCursor(txn, vertex.descriptor, nogdb::Condition("distance").contain("a"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_INVALID_COMPARATOR, "CTX_INVALID_COMPARATOR");
	...
	auto &edge = edges[0];
	try {
		auto edges = nogdb::Vertex::getOutEdgeCursor(txn, edge.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getOutEdgeCursor(txn, tmp, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_invalid_edge_all_cursor() {
	...
	auto &vertex = vertices[0];
	try {
		auto edges = nogdb::Vertex::getAllEdgeCursor(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		auto edges = nogdb::Vertex::getAllEdgeCursor(txn, vertex.descriptor, nogdb::Condition("name").eq("Andrew Street"), classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getAllEdgeCursor(txn, vertex.descriptor, nogdb::Condition("names").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_PROPERTY, "CTX_NOEXST_PROPERTY");
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getAllEdgeCursor(txn, vertex.descriptor, nogdb::Condition("distance").contain("a"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_INVALID_COMPARATOR, "CTX_INVALID_COMPARATOR");
	...
	auto &edge = edges[0];
	try {
		auto edges = nogdb::Vertex::getAllEdgeCursor(txn, edge.descriptor, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getAllEdgeCursor(txn, tmp, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_edge_in_cursor_condition_function() {
    ...
    try {
        ...
        auto res = nogdb::Vertex::getInEdgeCursor(txn, vertex, nogdb::Condition("name").eq("George Street"), nogdb::ClassFilter{"street"});
        assert(res.size() == 1);
        res = nogdb::Vertex::getInEdgeCursor(txn, vertex, test_condition_function_4, nogdb::ClassFilter{"street"});
        assert(res.size() == 2);
        cursorContains(res, std::set<std::string>{"Andrew Street", "George Street"}, "name");
    } catch (const nogdb::Error& ex) {
        std::cout << "\nError: " << ex.what() << std::endl;
    ...
    try {
        ...
        auto res = nogdb::Vertex::getInEdgeCursor(txn, vertex, nogdb::Condition("name").eq("Isaac Street"), classNames);
        assert(res.size() == 1);
        res = nogdb::Vertex::getInEdgeCursor(txn, vertex, test_condition_function_5, classNames);
        assert(res.size() == 3);
        cursorContains(res, std::set<std::string>{"Dwayne Way", "Henry Road", "Isaac Street"}, "name");
    } catch (const nogdb::Error& ex) {
        std::cout << "\nError: " << ex.what() << std::endl;
    ...
    try {
        ...
        auto res = nogdb::Vertex::getInEdgeCursor(txn, vertex, nogdb::Condition("name").eq("The Outer Ring C"));
        assert(res.size() == 1);
        res = nogdb::Vertex::getInEdgeCursor(txn, vertex, test_condition_function_6);
        assert(res.size() == 3);
        cursorContains(res, std::set<std::string>{"Eastern Way", "Fisher Avenue", "The Outer Ring C"}, "name");
    } catch (const nogdb::Error& ex) {
        std::cout << "\nError: " << ex.what() << std::endl;
...
void test_find_edge_out_cursor_condition_function() {
    ...
    try {
        ...
        auto res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, nogdb::Condition("name").eq("Andrew Street"), nogdb::ClassFilter{"street"});
        assert(res.size() == 1);
        res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, test_condition_function_7, nogdb::ClassFilter{"railway"});
        assert(res.size() == 3);
        cursorContains(res, std::set<std::string>{"Bamboo Way", "Catalina Way", "Dwayne Way"}, "name");
        res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, test_condition_function_8, nogdb::ClassFilter{"railway"});
        assert(res.size() == 2);
        cursorContains(res, std::set<std::string>{"Andy Way", "Catalina Way"}, "name");
        res = nogdb::Vertex::getOutEdgeCursor(txn, vertex, test_condition_function_9, nogdb::ClassFilter{"railway"});
        assert(res.size() == 1);
        res.first(); assert(res->record.get("name").toText() == "Dwayne Way");
    } catch (const nogdb::Error& ex) {
        std::cout << "\nError: " << ex.what() << std::endl;
        assert(false);
    }
...
void test_find_edge_all_cursor_condition_function() {
    ...
    try {
        ...
        auto &vertex = vertices->descriptor;
        auto classNames = nogdb::ClassFilter{"street", "railway"};
        auto res = nogdb::Vertex::getAllEdgeCursor(txn, vertex, test_condition_function_10, classNames);
        assert(res.size() == 1);
        res.next(); assert(res->record.get("name").toText() == "Gravity Way");
        res = nogdb::Vertex::getAllEdgeCursor(txn, vertex, test_condition_function_11, classNames);
        assert(res.size() == 5);
    } catch (const nogdb::Error& ex) {
        std::cout << "\nError: " << ex.what() << std::endl;
        assert(false);
...
void test_find_invalid_edge_in_cursor_condition_function() {
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getInEdgeCursor(txn, vertex.descriptor, condition, nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		auto edges = nogdb::Vertex::getInEdgeCursor(txn, vertex.descriptor, condition, classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	auto &edge = edges[0];
	try {
		auto edges = nogdb::Vertex::getInEdgeCursor(txn, edge.descriptor, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getInEdgeCursor(txn, tmp, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_invalid_edge_out_cursor_condition_function() {
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getOutEdgeCursor(txn, vertex.descriptor, condition, nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		auto edges = nogdb::Vertex::getOutEdgeCursor(txn, vertex.descriptor, condition, classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	auto &edge = edges[0];
	try {
		auto edges = nogdb::Vertex::getOutEdgeCursor(txn, edge.descriptor, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getOutEdgeCursor(txn, tmp, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_invalid_edge_all_cursor_condition_function() {
	...
	txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_ONLY};
	try {
		auto edges = nogdb::Vertex::getAllEdgeCursor(txn, vertex.descriptor, condition, nogdb::ClassFilter{"streets"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	try {
		auto classNames = nogdb::ClassFilter{"street", "railway", "subway"};
		auto edges = nogdb::Vertex::getAllEdgeCursor(txn, vertex.descriptor, condition, classNames);
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_NOEXST_CLASS, "CTX_NOEXST_CLASS");
	...
	auto &edge = edges[0];
	try {
		auto edges = nogdb::Vertex::getAllEdgeCursor(txn, edge.descriptor, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch (const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, CTX_MISMATCH_CLASSTYPE, "CTX_MISMATCH_CLASSTYPE");
	...
	try {
		auto tmp = vertex.descriptor;
		tmp.rid.second = -1;
		auto res = nogdb::Vertex::getAllEdgeCursor(txn, tmp, condition, nogdb::ClassFilter{"street"});
		assert(false);
	} catch(const nogdb::Error& ex) {
		txn.rollback();
		REQUIRE(ex, GRAPH_NOEXST_VERTEX, "GRAPH_NOEXST_VERTEX");
...
void test_find_edge_out_cursor_with_expression() {
    ...
    try {
        ...
        assert(res.size() == 1);
        res.next(); assert(res->record.get("name").toText() == "Henry Road");
