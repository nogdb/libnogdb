...
void test_get_invalid_vertices() {
	...
	auto txn = nogdb::Txn{*ctx, nogdb::Txn::Mode::READ_WRITE};
	try {
		auto v = nogdb::Vertex::create(txn, "books", nogdb::Record{}.set("title", "Percy Jackson").set("pages", 456).set("price", 24.5));
...
void test_get_edge_in_cursor() {
	...
	try {
		...
		assert(in_edges.size() == 2);
		in_edges.next(); assert(in_edges->record.get("time_used").toIntU() == 180U);
		in_edges.next(); assert(in_edges->record.get("time_used").toIntU() == 365U);
		in_edges = nogdb::Vertex::getInEdgeCursor(txn, v2_2);
		assert(in_edges.size() == 1);
		in_edges.first(); assert(in_edges->record.get("time_used").toIntU() == 430U);
		in_edges.last(); assert(in_edges->record.get("time_used").toIntU() == 430U);
	} catch(const nogdb::Error& ex) {
		std::cout << "\nError: " << ex.what() << std::endl;
		assert(false);
	}
...
void test_get_edge_out_cursor() {
    ...
    try {
        ...
        assert(out_edges.size() == 1);
        out_edges.first(); assert(out_edges->record.get("time_used").toIntU() == 365U);
        out_edges = nogdb::Vertex::getOutEdgeCursor(txn, v1_2);
        assert(out_edges.size() == 1);
        out_edges.next(); assert(out_edges->record.get("time_used").toIntU() == 180U);
        out_edges = nogdb::Vertex::getOutEdgeCursor(txn, v1_3);
        assert(out_edges.size() == 1);
        out_edges.to(0); assert(out_edges->record.get("time_used").toIntU() == 430U);
...
void test_get_edge_all_cursor() {
    ...
    try {
        ...
        assert(all_edges.size() == 1);
        all_edges.first(); assert(all_edges->record.get("time_used").toIntU() == 365U);
        all_edges = nogdb::Vertex::getAllEdgeCursor(txn, v1_2);
        assert(all_edges.size() == 1);
        all_edges.to(0); assert(all_edges->record.get("time_used").toIntU() == 180U);
        all_edges = nogdb::Vertex::getAllEdgeCursor(txn, v1_3);
        assert(all_edges.size() == 1);
        all_edges.last(); assert(all_edges->record.get("time_used").toIntU() == 430U);
        all_edges = nogdb::Vertex::getAllEdgeCursor(txn, v2_1);
        assert(all_edges.size() == 2);
        all_edges.to(0); assert(all_edges->record.get("time_used").toIntU() == 365U);
        all_edges.to(1); assert(all_edges->record.get("time_used").toIntU() == 180U);
        all_edges = nogdb::Vertex::getAllEdgeCursor(txn, v2_2);
        assert(all_edges.size() == 1);
        all_edges.next(); assert(all_edges->record.get("time_used").toIntU() == 430U);
    } catch(const nogdb::Error& ex) {
        std::cout << "\nError: " << ex.what() << std::endl;
        assert(false);
    }
