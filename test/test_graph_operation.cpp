	...
	try {
		for(const auto& res: nogdb::Vertex::get(txn, "folders")) {
			switch(res.record.get("name").toText().c_str()[0]) {
				case 'A': A = res.descriptor; break;
				case 'B': B = res.descriptor; break;
				case 'C': C = res.descriptor; break;
				case 'D': D = res.descriptor; break;
				case 'E': E = res.descriptor; break;
				case 'F': F = res.descriptor; break;
				case 'G': G = res.descriptor; break;
				case 'H': H = res.descriptor; break;
				case 'Z': Z = res.descriptor; break;
			}
		}
...
void test_bfs_traverse_out() {
	...
	try {
		...
		auto res = nogdb::Traverse::outEdgeBfs(txn, B, 1, 1, nogdb::ClassFilter{classNames.cbegin(), classNames.cend()});
		assert(res.size() == 3);
...
void test_bfs_traverse_all() {
	...
	try {
		...
		for(const auto& res: nogdb::Traverse::allEdgeBfs(txn, A, 1, 1, nogdb::ClassFilter{classNames.cbegin(), classNames.cend()})) {
			auto name = res.record.get("name").toText();
...
void test_bfs_traverse_in_cursor() {
	...
	try {
		...
		while(rsCursor.next()){
			switch(rsCursor->record.getText("name").c_str()[0]) {
				case 'A': A = rsCursor->descriptor; break;
				case 'B': B = rsCursor->descriptor; break;
				case 'C': C = rsCursor->descriptor; break;
				case 'D': D = rsCursor->descriptor; break;
				case 'E': E = rsCursor->descriptor; break;
				case 'F': F = rsCursor->descriptor; break;
				case 'G': G = rsCursor->descriptor; break;
				case 'H': H = rsCursor->descriptor; break;
				case 'Z': Z = rsCursor->descriptor; break;
			}
		}
	...
	try {
		...
		rsCursor.next(); assert(rsCursor->record.get("name").toText() == "F");
		rsCursor.next(); assert(rsCursor->record.get("name").toText() == "C");
		rsCursor.next(); assert(rsCursor->record.get("name").toText() == "A");

		rsCursor = nogdb::Traverse::inEdgeBfsCursor(txn, f, 1, 4, nogdb::ClassFilter{"link"});
		assert(rsCursor.size() == 4);
		rsCursor.next(); assert(rsCursor->record.get("name").toText() == "G");
		rsCursor.next(); assert(rsCursor->record.get("name").toText() == "E");
		rsCursor.next(); assert(rsCursor->record.get("name").toText() == "B");
		rsCursor.next(); assert(rsCursor->record.get("name").toText() == "A");
	...
	try {
		auto classNames = std::set<std::string>{"link", "symbolic"};
		auto rsCursor = nogdb::Traverse::inEdgeBfsCursor(txn, b, 0, 1, nogdb::ClassFilter{classNames.cbegin(), classNames.cend()});
...
void test_bfs_traverse_out_cursor() {
	...
	try {
		auto classNames = std::vector<std::string>{"link", "symbolic"};
		auto rsCursor = nogdb::Traverse::outEdgeBfsCursor(txn, B, 1, 1, nogdb::ClassFilter{classNames.cbegin(), classNames.cend()});
		assert(rsCursor.size() == 3);
...
void test_bfs_traverse_all_cursor() {
	...
	try {
		auto classNames = std::list<std::string>{"link", "symbolic"};
		auto rsCursor = nogdb::Traverse::allEdgeBfsCursor(txn, A, 1, 1, nogdb::ClassFilter{classNames.cbegin(), classNames.cend()});
		while(rsCursor.next()) {
			auto name = rsCursor->record.get("name").toText();
			assert(name == "B" || name == "a" || name == "C" || name == "D");
		}
...
void test_shortest_path_cursor() {
	...
	try {
		...
		assert(res.size() == 1);
		res.next(); assert(res->record.get("name").toText() == "a");

		res = nogdb::Traverse::shortestPathCursor(txn, f, f);
		assert(res.size() == 1);
		res.next(); assert(res->record.get("name").toText() == "f");

		res = nogdb::Traverse::shortestPathCursor(txn, B, B);
		assert(res.size() == 1);
		res.next(); assert(res->record.get("name").toText() == "B");
...
void test_bfs_traverse_cursor_with_condition() {
	...
	try {
		...
		assert(res.size() == 1);
		res.first(); assert(res->record.get("name").toText() == "F");
